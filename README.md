...
这是一个命令行和图形界面的股票、外汇和加密货币报价查看工具。它可以从腾讯获取股票数据，从新浪财经获取外汇数据，从528btc获取加密货币数据，并在终端或图形界面中以表格形式展示。
...
- 修复命令行版本中键盘输入监听在部分 Linux 系统上的兼容性问题
- 优化图形界面版本的错误处理机制，增强程序稳定性
- 更新默认自选股列表，添加 IBIT (iShares Bitcoin Trust ETF)
